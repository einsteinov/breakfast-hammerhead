...
static u32 sync_threshold;
module_param(sync_threshold, uint, 0644);

static u32 input_boost_ms = 40;
module_param(input_boost_ms, uint, 0644);

static u32 migration_load_threshold = 15;
...
static int set_input_boost_freq(const char *buf, const struct kernel_param *kp)
{
	...
	if (!ntokens) {
		ret = sscanf(buf, "%u\n", &val);
		if (ret != 1 || (s32)val < 0 || val > UINT_MAX)
			return -EINVAL;
	...
	for (i = 0; i < ntokens; i += 2) {
		...
		if (ret != 2 ||
		   (s32)cpu < 0 || cpu > num_possible_cpus() ||
		   (s32)val < 0 || val > UINT_MAX)
			return -EINVAL;
...
static int get_input_boost_freq(char *buf, const struct kernel_param *kp)
{
	...
	for_each_possible_cpu(cpu) {
		...
		len += scnprintf(buf + len, sizeof(u32) * 16 + 2,
...
{
	int dest_cpu = (long)data, src_cpu, ret;
...
static __always_inline void cpu_boost_threads_stop(void)
{
	struct cpu_sync *s;
	int cpu;
