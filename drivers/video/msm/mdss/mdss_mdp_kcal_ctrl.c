...
static inline ssize_t mdss_mdp_kcal_display_commit(void)
{
...
static void mdss_mdp_kcal_update_pcc(struct kcal_lut_data *lut_data)
{
	...
	lut_data->red = lut_data->red > lut_data->min ?
			lut_data->red : lut_data->min;
	lut_data->green = lut_data->green > lut_data->min ?
			  lut_data->green : lut_data->min;
	lut_data->blue = lut_data->blue > lut_data->min ?
			 lut_data->blue : lut_data->min;
...
static DEVICE_ATTR(_name, S_IRUGO | S_IWUSR, show_##_name, store_##_name);

#define define_one_kcal_node(_name, object, down_limit, up_limit, update_pa) \
...
{									     \
	struct kcal_lut_data *lut_data = dev_get_drvdata(dev);		     \
									     \
	return scnprintf(buf, sizeof(uint32_t) * 8 + 1,			     \
			 "%u\n", lut_data->object);			     \
}									     \
									     \
...
{									     \
	...
	if (ret || (int32_t)val < down_limit || val > up_limit ||	     \
	    lut_data->object == val)					     \
		return -EINVAL;						     \
									     \
	lut_data->object = val;						     \
			 ...
			 char *buf)
{
	...
	return scnprintf(buf, sizeof(uint32_t) * 24 + 3, "%u %u %u\n",
...
static int kcal_ctrl_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(lut_data)) {
		pr_err("failed to allocate memory for LUT data\n");
		return -ENOMEM;
	}
